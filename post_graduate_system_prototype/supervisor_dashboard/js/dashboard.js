import { qs, qsa, getSupervisorSession, toast, STAGES, navigateTo } from './main.js';
...
export function initDashboard() {
  ...
  fetchAndRenderDashboard(session);
}

function supervisorIdentifiers(session) {
  return [session?.id, session?.fullName, session?.name, session?.userNumber]
    .filter(Boolean)
    .map((value) => String(value));
}

function resolveSupervisorSlot(student, session) {
  const identifiers = supervisorIdentifiers(session);
...
async function fetchAndRenderDashboard(session) {
  const root = qs("#student-table-container");
  try {
    const students = await api.getAssignedStudents(session.id || session.fullName);
    renderDashboard(students, session);
  } catch (error) {
    console.error(error);
    root.innerHTML = `<div class="alert alert-error animate-in mx-12 my-12"><span class="alert-icon">!</span><div>Error: ${error.message}</div></div>`;
  }
}

function renderDashboard(students, session) {
  const root = qs("#student-table-container");
  ...
  root.innerHTML = `
    <div class="table-container">
      <table class="data-table">
         ...
         <tbody id="student-table-body">
            ${students.length ? students.map((student) => renderStudentRow(student, session)).join('') : `
    ...
    <div class="p-6 bg-grey-100 text-[10px] font-bold text-grey-500 uppercase tracking-widest flex justify-between">
        ...
        <span>Gatekeeper Integrity: Verified</span>
    </div>
  `;
...
function renderStudentRow(student, session) {
  const slot = resolveSupervisorSlot(student, session);
...
function setupDashboardEvents(session) {
  ...
  qsa(".btn-accept").forEach((btn) => {
    btn.onclick = (event) => {
      ...
      handleAssignment(btn.dataset.id, session, "accepted");
    };
  });
  ...
  qsa(".btn-reject").forEach((btn) => {
    btn.onclick = (event) => {
      event.stopPropagation();
      handleAssignment(btn.dataset.id, session, "rejected");
    };
  });
  ...
  qsa(".btn-manage").forEach((btn) => {
    btn.onclick = (event) => {
      event.stopPropagation();
      navigateTo('student-detail', null, btn.dataset.id);
    };
  });
...
async function handleAssignment(studentId, session, action) {
  try {
    toast(`Processing assignment ${action}...`, { tone: "blue" });
    await api.updateAssignmentStatus(studentId, session.id || session.fullName, action);
    toast(`Successfully ${action} student assignment`, { tone: action === "accepted" ? "green" : "red" });
    initDashboard();
  } catch (error) {
    toast(`Error: ${error.message}`, { tone: "red" });
  }
}
